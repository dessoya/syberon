...
DWORD WINAPI LuaThreadFunction2(LPVOID lpParam) {

	_set_se_translator(MiniDumpFunction);
	try  // this try block allows the SE translator to work
	{
		auto p = (LuaThreadParams2 *)lpParam;
		auto a = p->a;
		LuaScript *L = new LuaScript();

		if (L->executeFile(p->filename)) {
			eprint("Error in file " + p->filename)
			eprint(L->getError());
			return 1;
		}

		if (L->executeFunction("thread", a)) {
			eprint("Error in function 'thread' in file " + p->filename);
			eprint(L->getError());
			return 1;
		}
...
static int luaC_Thread_NewArgs(lua_State *L) {

	auto a = new LuaArgumants(L, 1);
	auto p = new LuaThreadParams2{ (char *)lua_tostring(L, 1), a };

	HANDLE ht = CreateThread(NULL, 0, LuaThreadFunction2, p, 0, NULL);
...
static int luaC_Thread_PostMessage(lua_State *L) {
	...
	if (lua_isnumber(L, 1)) {
		...
		if (r == 0) {
			...
			// eprint("Error thread " + inttostr(e) + " message " + inttostr(messageId));
...
static int luaC_Thread_id(lua_State *L) {
	lua_pushinteger(L, GetCurrentThreadId());
	return 1;
}

static int luaC_Thread_yield(lua_State *L) {
	boost::thread::yield();
	return 0;
}
...
void lm_Thread_install(lua_State* _l) {
	...
	lua_register(_l, "C_Thread_NewArgs", luaC_Thread_NewArgs);
	...
	*/
	...
	lua_register(_l, "C_Thread_id", luaC_Thread_id);

	lua_register(_l, "C_Thread_yield", luaC_Thread_yield);
